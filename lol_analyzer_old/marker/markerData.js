export class markerData {
	constructor() {
		this.cleaningList = ['champion', 'non-minion'];
		let ignoreMarkerWords = [
			'mana',
			'minimum',
			'reduced',
			'energy',
		...
		this.baseCCWords = ['knock up', 'root', 'slow', 'stun', 'knockback', 'disable'];

		this.enhancerWords = ['maximum', 'bonus', 'reduction', 'percentage'];
		this.obilityWords = ['movement speed', 'impared movement'];

		this.fillerWords = ['strengh', 'strength', 'modifier'];
		this.metaWords = ['duration', 'per tick'];

		this.targetWords = ['champion', 'non-minion'];

		this.ignoreWords = ['IGNORE THIS'];

		this.combWords = [];
		this.searchMarkers = [/mixed magic damage/i, /(mixed).*?(damage)/i];
	}
}
