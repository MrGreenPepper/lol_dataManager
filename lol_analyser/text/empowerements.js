...
function empoweringBasicRegexGenerator(championName) {
	championName = tools.basicStringClean(championName);
	let structureRegexs = [];
	structureRegexs.push(`${championName}.{0,20}gains`);
	structureRegexs.push(`${championName}.{0,6}is empowered`);
	structureRegexs.push(`(${championName}).{0,6}(empowers).{0,6}(his)|(her).{0,6}`);
