const extractorTools = require('./extractorTools');
async function extractSkillTabs(championData) {
  ...
  for (let i = 0; i < abilityNumbers; i++) {
    //console.log('textContent SkillTabs:');
    let textContentCount = Object.keys(championData.abilities[i].textContent);
    for (let t = 0; t < textContentCount.length; t++) {
      //empty != undefined
      if (championData.abilities[i].textContent[t].skillTabs != undefined) {
        let textContentSkillTabCount = Object.keys(
          championData.abilities[i].textContent[t].skillTabs
        );
        for (let s = 0; s < textContentSkillTabCount.length; s++) {
          championData.abilities[i].textContent[t].skillTabs[s] = await divideSkillTabs(
            championData.abilities[i].textContent[t].skillTabs[s]
          );
        }
      }
    }
...
module.exports.extractSkillTabs = extractSkillTabs;
