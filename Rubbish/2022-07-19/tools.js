...
export async function mergeData(newData) {
	...
	let oldData = loadJSONData(`./data/${dataSet_name}`);

	//go as deep as you can and check if a key has value, if both (old and newKey) have values take the new one
	let newKews;

	const goDepper = function (heavyObject) {
		let objectKeys = Object.keys(heavyObject);
		if (objectKeys.length > 0) {
		}
	};
	const exchangeValues = function (pairOld, pairNew) {
		let oldHasValue = false;
