...
export async function getInGameData() {
	...
	for (let championName of championList) {
		try {
			...
			championData.scraped_data.inGameData.skillOrder = championRawData.abilities.skillOrder;
			championData.scraped_data.inGameData.summonerSpells = championRawData.summonerSpells;

			await tools.saveJSONData(
				championData,
				`./lol_scraper/data/champions/inGameData/${championData.name}_data.json`
			);

			await tools.mergeData(championName, championData);
