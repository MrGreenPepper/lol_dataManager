...
export async function mergeData(championName, newData) {
	let oldData = await loadJSONData(`./data/champions/${championName}_data.json`);

	let currentData = { ...oldData, newData };
	await saveJSONData(currentData, `./data/champions/${championName}_data.json`);
	return;
}
